...
- **Prototype Map Review:** load AI-generated prototype maps via `load_prototype_map_for_test()` for visual review.
...
## Prototype Map Review (AI-generated maps)

Debug/editor builds only. **Does not promote maps to playable** and does not change the production map dropdown.

### How to open

1. Run from the **editor** or a **debug export**.
2. Load into the race scene (`main_game.tscn` via **START**).
3. Press **F3** to open the Dev Control Panel.
4. Scroll to **Prototype Map Review**.
...
Standalone headless check:

```bash
godot --headless --path . -s res://scripts/debug/prototype_map_review_test.gd
```
