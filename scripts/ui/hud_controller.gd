...
func _ready() -> void:
	...
	GameEventBus.instance().round_stats_changed.connect(_on_round_stats_changed)
	...
	GameEventBus.instance().join_rejected.connect(_on_join_rejected)
	GameEventBus.instance().join_accepted_late.connect(_on_join_accepted_late)
	GameEventBus.instance().post_round_auto_reset_tick.connect(_on_post_round_auto_reset_tick)
	...
	GameEventBus.instance().zombie_status_changed.connect(_on_zombie_status_changed)
	GameEventBus.instance().leader_changed.connect(_on_leader_changed)
	GameEventBus.instance().command_text_changed.connect(_on_command_text_changed)
