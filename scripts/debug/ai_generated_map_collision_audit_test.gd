...
func _test_focus_map(map_id: String) -> void:
	...
	var safe_floor: Node = map_root.get_node_or_null("GameplayLayer/SafeFloor")
	if safe_floor != null:
			...
			for child in plate.get_children():
				if child is CollisionShape3D and not (child as CollisionShape3D).disabled:
					...
					if not ((child as CollisionShape3D).shape is BoxShape3D):
						_fail("[%s] SafeFloor should use BoxShape3D collision" % map_id)
			if not shape_found:
				_fail("[%s] SafeFloor plate missing enabled collision shape" % map_id)
