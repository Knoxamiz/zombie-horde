...
func _test_layout_shape() -> void:
	...
	var desert_surface_pieces: Array = layout.get("supplemental_surface_pieces", [])
	...
	if desert_surface_pieces.size() != surface_pieces.size() * 6:
		_fail("Straight Descent should provide solid walk surfaces under every desert tier")
...
func _test_scene_builds_surfaces() -> void:
	...
	elif surfaces.get_child_count() < 49:
		_fail("Straight Descent should build its road and all six desert-side surfaces")
